...
| 4-2 | FCM 푸시 알림 발송 | ⬜ 예정 | 중 | Firebase Admin SDK 서버 전용 (Secret 등록 완료) |
| 4-3 | 카카오 알림톡 발송 | ⬜ 예정 | 중 | API 키 보호 (Secret 등록 완료) |
...
| `FIREBASE_SERVICE_ACCOUNT_JSON` | FCM 푸시 알림용 Firebase 서비스 계정 (JSON 전체) | send-push, send-chat-message, create-payment-request, complete-care, scheduler | 2026-04-14 |

#### 추가 등록 필요 (Step 4 진행 시)

| Secret Name | 용도 | 사용하는 Edge Function | 확보 방법 |
|-------------|------|----------------------|----------|
| `INICIS_MID` | 이니시스 상점 ID (MID) | inicis-callback | 외주 개발자에게 확인 |
| `INICIS_SIGN_KEY` | 이니시스 서명 키 (위변조 검증용, 사용 여부 확인 필요) | inicis-callback | 외주 개발자에게 확인 |
| `JUSO_CONFM_KEY` | 행안부 주소 API 승인키 | address-proxy | PHP에서 확인됨: `devU01TX0FVVEgyMDI1MTEwOTIwMjUyNjExNjQyMjQ=` |
| `NAVER_MAP_CLIENT_ID` | 네이버 역지오코딩 API Client ID | address-proxy | 외주 개발자에게 확인 (naver-address.php 미수령) |
| `NAVER_MAP_CLIENT_SECRET` | 네이버 역지오코딩 API Client Secret | address-proxy | 외주 개발자에게 확인 |

> **참고**: `SUPABASE_URL`과 `SUPABASE_ANON_KEY`는 Supabase가 기본 제공하므로 별도 등록 불필요.
...
| 2026-04-14 | **보안 조치 완료** — wooyoopet-backend 저장소 삭제, Firebase 서비스 키 교체 완료, Supabase Secrets 3개 등록 (KAKAO_ALIMTALK_API_KEY, KAKAO_ALIMTALK_USER_ID, FIREBASE_SERVICE_ACCOUNT_JSON), 추가 등록 필요 Secret 5개 식별 (INICIS_MID, INICIS_SIGN_KEY, JUSO_CONFM_KEY, NAVER_MAP_CLIENT_ID, NAVER_MAP_CLIENT_SECRET), 섹션 9-5 Secrets 관리 가이드 추가 |
