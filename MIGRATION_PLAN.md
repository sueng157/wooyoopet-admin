...
| VIEW 이름 | 기저 테이블 | 노출 컬럼 수 | 제외 항목 | SQL 파일 |
|-----------|-----------|------------|----------|---------|
| `members_public_profile` | members | 11 | 전화번호, 상세주소, 생년월일, 성별, 통신사, 본인인증, 노쇼제재, 정지/탈퇴 사유, 알림설정, 앱설정 | sql/44_00 |
| `pets_public_info` | pets | 15 (+ `WHERE deleted=false`) | deleted, created_at, updated_at | sql/44_00 |
| `settlement_infos_public` | settlement_infos | 4 (id, member_id, kindergarten_id, inicis_status) | 사업자정보, 계좌정보, 주민번호, 개인정보 전체 | sql/44_00 |
...
- 타 회원 데이터 JOIN: 원본 테이블 대신 **공개 VIEW** 사용 (`members_public_profile`, `pets_public_info`, `settlement_infos_public`)
...
|---|---------|--------|---------|------|----------|----------|------|
| 1 | sql/44_01 | `app_get_kindergarten_detail` | get_partner.php | 유치원 상세 | kindergartens + members + favorite + settlement_infos + pets JOIN | ✅ 3개 모두 | ⬜ |
| 2 | sql/44_02 | `app_get_kindergartens` | get_partner_list.php | 유치원 목록 | kindergartens + review COUNT + Haversine 거리 정렬 + 페이지네이션 | ✅ members, settlement_infos | ⬜ |
...
| 2 | 11 | `app_get_education_with_progress` | ★★☆ | ✅ | 독립적, education 테이블만 |
| 3 | 1 | `app_get_kindergarten_detail` | ★★★ | ✅ | 5테이블 JOIN, VIEW 3개 사용 |
| 4 | 2 | `app_get_kindergartens` | ★★★ | ✅ | 목록, Haversine 거리, 페이지네이션 |
