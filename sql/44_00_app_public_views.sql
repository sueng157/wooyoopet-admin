-- ============================================================
-- SQL 44-0: internal 스키마 VIEW 3개 생성 (RLS 충돌 해결용)
-- ============================================================
...
--
-- [보안: internal 스키마 사용]
-- VIEW를 public 스키마가 아닌 internal 스키마에 생성한다.
-- Supabase PostgREST는 public 스키마만 REST API endpoint로 노출하므로,
-- internal 스키마의 VIEW는 API endpoint가 생성되지 않는다.
-- → 로그인한 사용자가 Postman/curl로 VIEW를 직접 조회하는 것이 구조적으로 불가능.
-- → RPC 함수 내부에서만 internal.members_public_profile 등으로 참조 가능.
--
...
-- ============================================================
-- STEP 0: internal 스키마 생성
-- ============================================================
CREATE SCHEMA IF NOT EXISTS internal;

COMMENT ON SCHEMA internal IS
  'PostgREST 비노출 스키마 — RPC 전용 VIEW, 헬퍼 함수 등. '
  'public 스키마와 달리 REST API endpoint가 생성되지 않아 직접 조회 불가.';


-- ============================================================
-- VIEW 1: internal.members_public_profile
-- ============================================================
...
DROP VIEW IF EXISTS internal.members_public_profile;

CREATE VIEW internal.members_public_profile
...
-- authenticated 역할에 internal 스키마 USAGE + VIEW SELECT 허용
GRANT USAGE ON SCHEMA internal TO authenticated;
GRANT SELECT ON internal.members_public_profile TO authenticated;


-- ============================================================
-- VIEW 2: internal.pets_public_info
-- ============================================================
...
DROP VIEW IF EXISTS internal.pets_public_info;

CREATE VIEW internal.pets_public_info
...
COMMENT ON VIEW internal.pets_public_info IS
  '타 회원 반려동물 VIEW — 삭제된 레코드 제외, 내부 관리 컬럼 제외. '
  'internal 스키마: PostgREST API endpoint 미노출 → RPC 내부에서만 접근 가능. '
  'RPC에서 internal.pets_public_info로 참조. '
  '원본: MIGRATION_PLAN.md 섹션 2.5 공통 RLS 충돌 해결 방안 A';

GRANT SELECT ON internal.pets_public_info TO authenticated;


-- ============================================================
-- VIEW 3: internal.settlement_infos_public
-- ============================================================
...
DROP VIEW IF EXISTS internal.settlement_infos_public;

CREATE VIEW internal.settlement_infos_public
  WITH (security_invoker = false)     -- = SECURITY DEFINER
AS
SELECT
  id,
  ...
  kindergarten_id,
  inicis_status
FROM public.settlement_infos;

COMMENT ON VIEW internal.settlement_infos_public IS
  '정산정보 VIEW — 유치원 활성화 상태(inicis_status)만 노출. '
  '금융정보(계좌, 사업자번호), 개인정보(주민번호, 전화번호) 완전 제외. '
  'internal 스키마: PostgREST API endpoint 미노출 → RPC 내부에서만 접근 가능. '
  'RPC에서 internal.settlement_infos_public로 참조. '
  '원본: MIGRATION_PLAN.md 섹션 2.5 공통 RLS 충돌 해결 방안 A';

GRANT SELECT ON internal.settlement_infos_public TO authenticated;
...
BEGIN
  ...
  RAISE NOTICE '  ※ VIEW는 internal 스키마 → PostgREST API 미노출 (직접 조회 불가)';
  RAISE NOTICE '  ※ RPC 함수에서 internal.{view_name}으로 참조';
END $$;
