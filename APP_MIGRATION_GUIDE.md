...
| #6 | `set_profile_update.php` | 자동 API + Storage | `members UPDATE + member-images` | R1 ✅ |
...
**태그 집계 구조**:
RPC는 7개 긍정 태그별 COUNT를 `tags[]` 배열로 반환합니다 (내부 구현: tag_counts CTE로 COUNT 선집계 후 json_agg 변환):
1. 상담이 친절하고 편안했어요
2. 사진과 영상을 자주 보내주셨어요
3. 아이 상태를 자세히 알려주셨어요
...
**태그 집계 정책**: `is_guardian_only=true` 후기도 태그 집계에는 포함 (비공개는 "내용"이지 "통계"가 아님). 내부 구현: tag_counts CTE로 COUNT 선집계 후 json_agg 변환.
...
| 2026-04-30 | **후기 RPC 태그 집계 집계함수 중첩 오류 수정** — §13-2(#44 `app_get_guardian_reviews`), §13-3(#44b `app_get_kindergarten_reviews`) 태그 집계 구조 설명에 내부 구현 변경 반영 (tag_counts CTE 분리). RPC 내부 SQL에서 `json_agg` 내부에 `COUNT` 직접 중첩 → PostgreSQL 'aggregate function calls cannot be nested' 오류 → `tag_counts` CTE 추가로 해결. 앱 호출 인터페이스(파라미터·응답 구조) 변경 없음 |
