...
[완료] JavaScript UI 구현 (4파일 621줄, 인라인 JS 0건, PR #39~#42)
  ↓
[완료] 백엔드 구축 Phase 1~3 완료 + 통합 테스트 버그 수정 (PR #57)
  ↓
[예정] Phase 4: 호스팅 전환 (Cloudflare Pages)
```
...
- **현재 단계**: Phase 1~3 완료 + 통합 테스트 버그 수정 완료 (PR #57), Phase 4 예정 (호스팅 전환)
...
js/api.js               828줄  (Supabase CRUD 래퍼, 포매터, 배지, 페이지네이션, 엑셀, 감사로그, 마스킹, 권한)
js/dashboard.js         244줄  (통계 카드, 승인 대기, 매출 요약, 활동 로그)
js/members.js           714줄  (목록·상세, 이용정지/해제, 주소인증, 엑셀)
js/kindergartens.js     598줄  (목록·상세, 영업상태, 서브몰, 노쇼)
js/pets.js              451줄  (목록·상세, 삭제, 태그집계, 크로스링크)
js/reservations.js      461줄  (목록·상세, 직권취소, 노쇼 — PR #57에서 async/await 리팩터링)
js/payments.js          497줄  (결제/환불 2탭, 상세, 위약금면제 — PR #57에서 FK힌트 수정)
js/settlements.js       538줄  (정산정보/내역 2탭, 이니시스, 보류/해제)
...
현재 **백엔드 구축 Phase 1~3 완료**, Phase 4 (호스팅 전환) 예정입니다.
PR #57에서 통합 테스트 중 발견된 데이터 미표시 버그가 전면 수정되었습니다.
...
| 3-4 | 유치원관리 연동 (2번) | ✅ 완료 | PR #53 — 목록+상세(11영역), 영업상태, 서브몰, 노쇼, 크로스링크 |
...
| 조인 테이블 검색 | 예약 목록에서 보호자/유치원/반려동물 이름 검색은 현재 미구현 (예약번호만 검색 가능) | 중 |
| autoBadge 커스텀 색상 | `autoBadge(text, customMap)` 두 번째 인자가 무시됨 — `size_class` 배지 등 gray로 표시 | 낮 |
