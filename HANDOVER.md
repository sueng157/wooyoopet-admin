...
[완료] JavaScript UI 구현 (4파일 621줄, 인라인 JS 0건, PR #39~#42)
  ↓
[진행중] 백엔드 구축·호스팅 전환·기존 서버 정리 (Phase 1,2 완료, Phase 3 예정)
```
...
- **현재 단계**: Phase 1,2 완료, Phase 3 예정 (API 연동)
...
| HTML | JS 참조 순서 |
|------|-------------|
| 전체 42페이지 | supabase-js CDN → supabase-client.js → auth.js → common.js → components.js |
| `education-*.html` (7개) | supabase-js CDN → supabase-client.js → auth.js → common.js → components.js → educations.js |
| `settings.html` | supabase-js CDN → supabase-client.js → auth.js → common.js → components.js → settings.js |
| `login.html` | supabase-js CDN → supabase-client.js → auth.js (+ 인라인 로그인 폼 스크립트) |
...
js/auth.js              360줄  (로그인/로그아웃, 세션 체크, 사이드바·헤더 프로필 동적 표시, 메뉴 접근 권한 제어)
