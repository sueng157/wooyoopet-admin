...
[완료] JavaScript UI 구현 (4파일 621줄, 인라인 JS 0건, PR #39~#42)
  ↓
[진행중] 백엔드 구축·호스팅 전환·기존 서버 정리 (Phase 1 완료, Phase 2 예정)
```
...
- **현재 단계**: Phase 1 완료, Phase 2 예정 (관리자 인증)
...
**Phase 2 작업 시 참고사항**:
- `admin_accounts` 테이블에 테스트 관리자 3명 존재 (최고관리자/일반관리자/조회전용)
- 권한은 `perm_*` 개별 컬럼 11개로 관리 (JSON 아님): `perm_members`, `perm_kindergartens`, `perm_pets`, `perm_reservations`, `perm_payments`, `perm_settlements`, `perm_chats`, `perm_reviews`, `perm_educations`, `perm_contents`, `perm_settings`
- 각 권한의 허용값: `'조회만'`, `'조회+수정'`, `'접근불가'`
- 관리자 role 허용값: `'최고관리자'`, `'일반관리자'`, `'조회전용'`
- 관리자 status 허용값: `'활성'`, `'비활성'`
- Supabase 접속정보는 보안상 저장소 미포함 — 채팅 세션에서 직접 제공 필요
- `admin_login_logs` 테이블 존재 (로그인 이력 기록용)
