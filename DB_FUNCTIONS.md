...
| 2026-05-06 | **DB 성능 최적화** — 핵심 인덱스 5개 생성(admin_accounts.auth_user_id, kindergartens.member_id, reservations.member_id, reservations.kindergarten_id, pets.member_id). 고빈도 테이블 11개 ANALYZE(admin_accounts, chat_room_members, kindergartens, reservations, chat_rooms, chat_messages, payments, settlements, refunds, education_completions, pets). 12개 테이블에서 `_all_admin`(FOR ALL)과 중복되는 `_select_admin`(FOR SELECT) 정책 12개 제거. 배경: admin_accounts Seq Scan 84,145회, reservations 26,028회 — 인덱스 부재 + RLS 중복이 원인 | `sql/53_01`, `sql/53_02`, `sql/53_03` |
| 2026-05-08 | **Storage 업로드 에러 수정 및 버킷 정리** — 앱 이미지 업로드 시 400 에러("new row violates row-level security policy") 해결. 원인: public 버킷의 INSERT 시 Supabase 내부 SELECT가 RLS에 차단됨 (SELECT 정책 누락). pet-images·kindergarten-images SELECT 정책 추가 (54_01). member-images SELECT/INSERT/UPDATE/DELETE 정책 4개 추가 (54_02). profile-images 정책 3개 삭제 + 버킷 Dashboard에서 수동 삭제 (54_03). review-images SELECT 정책 추가 (54_04). 기존 48_08에서 삭제했던 member-images 버킷을 앱이 실제 사용하는 것으로 확인되어 재활용 | `sql/54_01`~`sql/54_04` |
| 2026-05-17 | **앱 RPC 3종에 payment.id 필드 추가** — PG 결제취소 호출 시 식별자로 사용 가능하도록 `app_get_reservations_guardian`(#5) / `app_get_reservations_kindergarten`(#5b) / `app_get_reservation_detail`(#6)의 payment JSON 블록에 'id' 필드 1줄 추가. 응답 구조 변경 없음(중첩 구조 그대로) | `sql/44_05_patch_app_rpc_get_reservations_guardian.sql`, `sql/44_05b_patch_app_rpc_get_reservations_kindergarten.sql`, `sql/44_06_patch_app_rpc_get_reservation_detail.sql` |
| 2026-05-17 | **payments.status CHECK 제약 확장** — `결제완료/결제취소` → `돌봄요청/결제완료/결제취소` (모바일앱 결제내역 표시 상태 3종 사양 반영). 기존 `reservations.status='수락대기'`인 결제건은 '돌봄요청'으로 백필 | `sql/57_01_payments_status_check_add_request.sql` |
